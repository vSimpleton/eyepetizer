                ...
                Row(
                  children: <Widget>[
                    Text(
                      ...
                      style:
                      TextStyle(color: ColorConfig.hitTextColor, fontSize: 12),
                    ),
                    Expanded(
                      child: Text(
    ...
    List<Widget> widgetList = itemData.data.content.data.tags.map((tag) {
      return Container(
        ...
        decoration: BoxDecoration(
            color: ColorConfig.tabBgColor, borderRadius: BorderRadius.circular(4)),
        child: Text(
          tag.name,
  ...
  Widget _videoState() {
    return Row(
      ...
      children: <Widget>[
        ...
        Row(
          children: <Widget>[
            Icon(Icons.favorite_border, size: 20, color: ColorConfig.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(
                  '${model.data.content.data.consumption.collectionCount}',
                  style: TextStyle(fontSize: 12, color: ColorConfig.hitTextColor)),
            )
          ],
        ),
        // 评论
        Row(
          children: <Widget>[
            Icon(Icons.message, size: 20, color: ColorConfig.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
        ...
        Row(
          children: <Widget>[
            Icon(Icons.star_border, size: 20, color: ColorConfig.hitTextColor),
            Padding(
              padding: EdgeInsets.only(left: 10),
              child: Text(StringConfig.collect_text,
                  style: TextStyle(fontSize: 12, color: ColorConfig.hitTextColor)),
            ),
          ],
        ),
        // 分享
        IconButton(
          icon: Icon(Icons.share, color: ColorConfig.hitTextColor),
